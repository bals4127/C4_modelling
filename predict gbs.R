...
c4_aci<- function (Ci, PPFD = 1500, Tleaf = 25, VPMAX25 = 120, Vcmax = 60, 
                   Vpr = 100, alpha = 0.001, gbs = 0.003, O2 = 210, JMAX25 = 400, 
...
{  
  ...
  aciSS <- function(VPMAX25, Vcmax, gbs) {
    Photo_mod <- acifun_wrap(dat$Ci, PPFD = dat$PPFD,VPMAX25=VPMAX25,Vcmax = Vcmax, gbs = gbs, JMAX25  = 400, Rd = Rd, Tleaf = dat$Tleaf)
    SS <- sum((dat$ALEAF - Photo_mod)^2)
    return(SS)
  }
  
  aciSS(vpmax_guess, vcmax_guess, gbs_guess)
  
  d <- 0.3
  n <- 20
  
  gg <- expand.grid(VPMAX25 = seq(vpmax_guess * (1 - d),vpmax_guess* (1 + d), length = n),Vcmax = seq(vcmax_guess * (1 - d),vcmax_guess* (1 + d), length = n),
                   gbs= seq(gbs_guess * (1 - d),gbs_guess* (1 + d)))
  ## 
  m <- with(gg, mapply(aciSS,VPMAX25=VPMAX25, Vcmax = Vcmax, gbs=gbs))
  ii <- which.min(m)
 ...
 gbs_guess<- gg$gbs[ii]
  ...
  nlsfit <- nlsLM(ALEAF ~ acifun_wrap(Ci, PPFD = PPFD, VPMAX25= VPMAX25,Vcmax = Vcmax, gbs= gbs), 
                data = dat, control = nls.control(maxiter = 500, minFactor = 1/10000), 
                start = list(VPMAX25= vpmax_guess,Vcmax = vcmax_guess, gbs= gbs_guess))
  
  p <- coef(nlsfit)
  acirun <- c4_aci(Ci = dat$Ci,  VPMAX25 = p[[1]],  Vcmax  = p[[2]],gbs = p[[3]], PPFD = dat$PPFD, Tleaf = dat$Tleaf)
