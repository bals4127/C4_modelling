...
{  
  ...
  nlsfit <- nls(ALEAF ~ acifun_wrap(Ci, PPFD = PPFD, VPMAX25= VPMAX25,Vcmax = Vcmax, Rd=Rd), 
                          ...
                          ...) 
{
  ...
  if (addzeroline) 
    abline(h = 0, lty = 3)
  Constant<- round(x$pars[,1], 2)
  a<- as.data.frame(Constant)
  if (addlegend) {
    legend("topleft", c(expression(italic(A)[c]), expression(italic(V)[p]), 
                        "Limiting rate"), lty = 1, lwd = c(1, 1, 2), col = c("red", 
                                                                             "blue", "black"))
    legend("bottomright", paste(capture.output(a)), col = c("blue"))
  }
}
