...
c4_aci<- function (Ci, PPFD = 1800, Tleaf = 25, VPMAX25 = 120, Vcmax = 60, 
...
{  
  ...
  aciSS <- function(VPMAX25, Vcmax) {
    Photo_mod <- acifun_wrap(dat$Ci, PPFD = dat$PPFD,VPMAX25=VPMAX25,Vcmax = Vcmax, JMAX25  = 400, Rd = Rd, Tleaf = dat$Tleaf)
  ...
  p <- coef(nlsfit)
  acirun <- c4_aci(Ci = dat$Ci,  VPMAX25 = p[[1]],  Vcmax  = p[[2]], PPFD = dat$PPFD, Tleaf = dat$Tleaf)
